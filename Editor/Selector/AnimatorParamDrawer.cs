...
namespace YuzeToolkit.Attributes.Editor
{
    ...
    public class AnimatorParamDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            if (!AttributeHelperEditor.TryGetValue<Animator>(property, animatorParamAttribute.AnimatorName,
                    out var animatorController))
            {
                AttributeHelperEditor.DrawWarningMessage(rect, property.displayName + "(无法找到对应animator!)");
                EditorGUI.EndProperty();
                return;
            }
