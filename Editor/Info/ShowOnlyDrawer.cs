...
namespace YuzeToolkit.Attributes.Editor
{
    ...
    public class ShowOnlyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            // 获取路径对象和自己的对象
            var target = AttributeHelperEditor.GetValue(property, showPropertyAttribute.Path);
            var self = AttributeHelperEditor.GetValue(property, property.name);

            // 判断类型是否相同(或者是否可以继承
            if (target != null && self != null && self.GetType().IsInstanceOfType(target) &&
                AttributeHelperEditor.SetValue(property, property.name, target))
            {
                GUI.enabled = false;
                EditorGUI.PropertyField(rect, property, label, true);
                GUI.enabled = true;
